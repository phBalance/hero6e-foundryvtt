...
- Improved default FoundryVTT appendNumber. Adding single tokens to a scene that append a number to the token now ensure duplicate token names are not created. Does not change the default FoundryVTT behavior for adding multiple tokens during a paste operation.
