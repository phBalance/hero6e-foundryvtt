...
export class HeroSystem6eActor extends Actor {
  ...
  async modifyTokenAttribute(attribute, value, isDelta = false, isBar = true) {
    ...
    if (isBar) {
      if (isDelta)
        value = Math.clamped(-99, Number(current.value) + value, current.max); // a negative bar is typically acceptable
  ...
  canAct(uiNotice) {
    if (this.statuses.has("knockedOut")) {
      if (uiNotice)
        ui.notifications.error(`${this.name} is KNOCKED OUT and cannot act.`);
      return false;
    }

    if (this.statuses.has("stunned")) {
      if (uiNotice)
        ui.notifications.error(`${this.name} is STUNNED and cannot act.`);
      return false;
    }

    if (this.statuses.has("aborted")) {
      if (uiNotice)
        ui.notifications.error(`${this.name} has ABORTED and cannot act.`);
      return false;
    }
    ...
    const _str = this.appliedEffects
      ...
      .reduce(
        (partialSum, a) =>
          partialSum +
          parseInt(
            a.changes.find((o) => o.key === "system.characteristics.str.max")
              ?.value || 0
          ),
        0
      );
    const _con = this.appliedEffects
      ...
      .reduce(
        (partialSum, a) =>
          partialSum +
          parseInt(
            a.changes.find((o) => o.key === "system.characteristics.con.max")
              ?.value || 0
          ),
        0
      );
    const _dex = this.appliedEffects
      ...
      .reduce(
        (partialSum, a) =>
          partialSum +
          parseInt(
            a.changes.find((o) => o.key === "system.characteristics.dex.max")
              ?.value || 0
          ),
        0
      );
    const _body = this.appliedEffects
      ...
      .reduce(
        (partialSum, a) =>
          partialSum +
          parseInt(
            a.changes.find((o) => o.key === "system.characteristics.body.max")
              ?.value || 0
          ),
        0
      );
    const _ego = this.appliedEffects
      ...
      .reduce(
        (partialSum, a) =>
          partialSum +
          parseInt(
            a.changes.find((o) => o.key === "system.characteristics.ego.max")
              ?.value || 0
          ),
        0
      );
    ...
    switch (key.toLowerCase()) {
      ...
      case "spd":
        return (
          base +
          1 +
          parseFloat(parseFloat((charBase("DEX") + _dex) / 10).toFixed(1))
        );
    ...
    return Array.from(this.allApplicableEffects())
      .filter(
        (o) =>
          ...
          o.statuses.size === 0 &&
          o.flags?.XMLID &&
          getPowerInfo({ xmlid: o.flags?.XMLID, actor: this })?.duration ===
            "persistent"
      )
      .sort((a, b) => a.name.localeCompare(b.name));
  }
          ...
          if (this.id) {
            ...
            if (system.XMLID === "COMPOUNDPOWER") {
              for (let system2 of system.POWER) {
                ...
                const item2 = await HeroSystem6eItem.create(itemData2, {
                  parent: this,
                });
  ...
  async CalcActorRealAndActivePoints() {
    ...
    for (const powerInfo of powers) {
      realCost += parseInt(
        this.system.characteristics[powerInfo.key.toLowerCase()]?.realCost || 0
      );
    }
